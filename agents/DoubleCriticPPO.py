...
def _add_advantage_and_value_target_to_episode(episode, gamma, lambd):
    ...
    for transition in reversed(episode):
        td_err_0 = (
            transition["reward"][0]
            + (gamma * transition["nonterminal"] * transition["next_v_pred_0"])
            - transition["v_pred_0"]
        )
        adv_0 = td_err_0 + gamma * lambd * adv
        transition["adv_0"] = adv_0

        td_err_1 = (
            transition["reward"][1]
            + (gamma * transition["nonterminal"] * transition["next_v_pred_1"])
            - transition["v_pred_1"]
        )
        adv_1 = td_err_1 + gamma * lambd * adv
        transition["adv_1"] = adv_1

        transition["v_teacher_0"] = adv_0 + transition["v_pred_0"]
        transition["v_teacher_1"] = adv_1 + transition["v_pred_1"]
...
def _add_log_prob_and_value_to_episodes(
    ...
    with torch.no_grad(), pfrl.utils.evaluating(model):
        distribs, vs_pred_0, vs_pred_1 = model(states)
        _, next_vs_pred_0, next_vs_pred_1 = model(next_states)

        actions = torch.tensor([b["action"] for b in dataset], device=device)
        log_probs = distribs.log_prob(actions).cpu().numpy()
        vs_pred_0 = vs_pred_0.cpu().numpy().ravel()
        vs_pred_1 = vs_pred_1.cpu().numpy().ravel()
        next_vs_pred_0 = next_vs_pred_0.cpu().numpy().ravel()
        next_vs_pred_1 = next_vs_pred_1.cpu().numpy().ravel()

    for transition, log_prob, v_pred_0, v_pred_1, next_v_pred_0, next_v_pred_1 in zip(
        dataset, log_probs, vs_pred_0, vs_pred_1, next_vs_pred_0, next_vs_pred_1
    ):
        transition["log_prob"] = log_prob
        transition["v_pred_0"] = v_pred_0
        transition["v_pred_1"] = v_pred_1
        transition["next_v_pred_0"] = next_v_pred_0
        transition["next_v_pred_1"] = next_v_pred_1
...
def _compute_explained_variance(transitions):
    ...
    """
    t = [np.array([tr["v_teacher_0"] for tr in transitions]), np.array([tr["v_teacher_1"] for tr in transitions])]
    y = [np.array([tr["v_pred_0"] for tr in transitions]), np.array([tr["v_pred_1"] for tr in transitions])]
    vart = [np.var(t[0]), np.var(t[1])]
    if vart[0] == 0 or vart[1] == 0:
        return np.nan
    else:
        return float(1 - np.var(t[0] - y[0]) / vart[0]) + float(1 - np.var(t[1] - y[1]) / vart[1])
...
class DoubleCriticPPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        self.value_record_0 = collections.deque(maxlen=value_stats_window)
        self.value_record_1 = collections.deque(maxlen=value_stats_window)

        self.entropy_record = collections.deque(maxlen=entropy_stats_window)
        self.value_loss_record_0 = collections.deque(maxlen=value_loss_stats_window)
        self.value_loss_record_1 = collections.deque(maxlen=value_loss_stats_window)
    ...
    def _update(self, dataset):
        ...
        assert "v_teacher_0" in dataset[0]
        assert "v_teacher_1" in dataset[0]

        if self.standardize_advantages:
            all_advs_0 = torch.tensor([b["adv_0"] for b in dataset], device=device)
            all_advs_1 = torch.tensor([b["adv_1"] for b in dataset], device=device)

            std_advs_0, mean_advs_0 = torch.std_mean(all_advs_0, unbiased=False)
            std_advs_1, mean_advs_1 = torch.std_mean(all_advs_1, unbiased=False)
        ...
        for batch in _yield_minibatches(
            ...
            distribs, vs_pred_0, vs_pred_1 = self.model(states)

            advs_0 = torch.tensor(
                [b["adv_0"] for b in batch], dtype=torch.float32, device=device
            )
            advs_1 = torch.tensor(
                [b["adv_1"] for b in batch], dtype=torch.float32, device=device
            )
            if self.standardize_advantages:
                advs_0 = (advs_0 - mean_advs_0) / (std_advs_0 + 1e-8)
                advs_1 = (advs_1 - mean_advs_1) / (std_advs_1 + 1e-8)

            log_probs_old = torch.tensor(
                [b["log_prob"] for b in batch], dtype=torch.float, device=device,
            )
            vs_pred_old_0 = torch.tensor(
                [b["v_pred_0"] for b in batch], dtype=torch.float, device=device,
            )
            vs_pred_old_1 = torch.tensor(
                [b["v_pred_1"] for b in batch], dtype=torch.float, device=device,
            )
            vs_teacher_0 = torch.tensor(
                [b["v_teacher_0"] for b in batch], dtype=torch.float, device=device,
            )
            vs_teacher_1 = torch.tensor(
                [b["v_teacher_1"] for b in batch], dtype=torch.float, device=device,
            )

            # Same shape as vs_pred: (batch_size, 1)
            vs_pred_old_0 = vs_pred_old_0[..., None]
            vs_pred_old_1 = vs_pred_old_1[..., None]
            vs_teacher_0 = vs_teacher_0[..., None]
            vs_teacher_1 = vs_teacher_1[..., None]
            ...
            loss = self._lossfun(
                ...
                vs_pred_old_0=vs_pred_old_0,
                vs_pred_old_1=vs_pred_old_1,
                log_probs_old=log_probs_old,
                advs_0=advs_0,
                advs_1=advs_1,
                vs_teacher_0=vs_teacher_0,
                vs_teacher_1=vs_teacher_1,
            )
            loss.backward()
    ...
    def _lossfun(
        self, entropy, vs_pred_0, vs_pred_1, log_probs, vs_pred_old_0, vs_pred_old_1, log_probs_old, advs_0, advs_1, vs_teacher_0, vs_teacher_1
    ):
        ...
        loss_policy = -torch.mean(
            torch.min(
                prob_ratio * advs_0,
                torch.clamp(prob_ratio, 1 - self.clip_eps, 1 + self.clip_eps) * advs_0,
            ),
        ) + -torch.mean(
            torch.min(
                prob_ratio * advs_1,
                torch.clamp(prob_ratio, 1 - self.clip_eps, 1 + self.clip_eps) * advs_1,
            ),
        )

        if self.clip_eps_vf is None:
            loss_value_func_0 = F.mse_loss(vs_pred_0, vs_teacher_0)
            loss_value_func_1 = F.mse_loss(vs_pred_1, vs_teacher_1)
        else:
            clipped_vs_pred_0 = _elementwise_clip(
                vs_pred_0, vs_pred_old_0 - self.clip_eps_vf, vs_pred_old_0 + self.clip_eps_vf,
            )
            loss_value_func_0 = torch.mean(
                torch.max(
                    F.mse_loss(vs_pred_0, vs_teacher_0, reduction="none"),
                    F.mse_loss(clipped_vs_pred_0, vs_teacher_0, reduction="none"),
                )
            )

            clipped_vs_pred_1 = _elementwise_clip(
                vs_pred_1, vs_pred_old_1 - self.clip_eps_vf, vs_pred_old_1 + self.clip_eps_vf,
            )
            loss_value_func_1 = torch.mean(
                torch.max(
                    F.mse_loss(vs_pred_1, vs_teacher_1, reduction="none"),
                    F.mse_loss(clipped_vs_pred_1, vs_teacher_1, reduction="none"),
                )
            )

        loss_value_func = ((1 - self.value_ratio) * loss_value_func_0) + (self.value_ratio * loss_value_func_1)
        loss_entropy = -torch.mean(entropy)

        self.value_loss_record_0.append(float(loss_value_func_0))
        self.value_loss_record_1.append(float(loss_value_func_1))
    ...
    def _batch_act_eval(self, batch_obs):
        ...
        with torch.no_grad(), pfrl.utils.evaluating(self.model):
            if self.recurrent:
                ...
                action_distrib, _, _ = self.model(b_state)
    ...
    def _batch_act_train(self, batch_obs):
        ...
        with torch.no_grad(), pfrl.utils.evaluating(self.model):
            if self.recurrent:
                ...
                action_distrib, batch_value_0, batch_value_1 = self.model(b_state)
            batch_action = action_distrib.sample().cpu().numpy()
            self.entropy_record.extend(action_distrib.entropy().cpu().numpy())
            self.value_record_0.extend(batch_value_0.cpu().numpy())
            self.value_record_1.extend(batch_value_1.cpu().numpy())
    ...
    def get_statistics(self):
        return [
            ("average_value_0", _mean_or_nan(self.value_record_0)),
            ("average_value_1", _mean_or_nan(self.value_record_1)),
            ("average_entropy", _mean_or_nan(self.entropy_record)),
            ("average_value_loss_0", _mean_or_nan(self.value_loss_record_0)),
            ("average_value_loss_1", _mean_or_nan(self.value_loss_record_1)),
