...
class SharedAgent(Agent):
    ...
    def observe(self, observation, reward, done, info):
        ...
        if done:
            if info['eps'] % 10 == 0:
                model_path = self.config['log_dir']+'agent'+str(info['eps'])
                print("Save model at " + model_path)
                self.agent.save(model_path)



class IndependentAgent_CycleAct(Agent):
    def __init__(self, config, obs_act, map_name, thread_number):
        super().__init__()
        self.config = config
        self.agents = dict()
    ...
    def act(self, observation):
        if self.acts is None:
            self.acts = dict()
            for agent_id in observation.keys():
                self.acts[agent_id] = 0

        for agent_id in observation.keys():
            if self.agents[agent_id].act(observation[agent_id]) == 1:
                self.acts[agent_id] = (self.acts[agent_id] + 1) % self.act_sizes[agent_id]
    ...
    def observe(self, observation, reward, done, info):
        for agent_id in observation.keys():
            ...
            if done:
                if info['eps'] % 10 == 0:
                    self.agents[agent_id].save(self.config['log_dir']+'agent_'+str(info['eps'])+'_'+agent_id)
