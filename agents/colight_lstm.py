...
class CoLight_LSTM(IndependentAgent):
...
class CoLightAgent(nn.Module):
    def __init__(self, config, obs_space, act_space, device, adj, gats=None):
        ...
        self.obs_embed = nn.LSTM(self.obs_space, self.total_dim, batch_first=True, num_layers=2)
    ...
    def forward(self, x):
        ...
        ### Embedding ###
        obs = torch.permute(obs, (0, 2, 1, 3))  # batch, signals, n_history, feat
        obs = torch.reshape(obs, (batch_size * dim_signal, dim_history, dim_feat))            
        # signals * batch, n_history, feat

        out, _ = self.obs_embed(obs)
        out = out[:, -1, :]        # signals * batch, feat
        out = torch.reshape(out, (batch_size, dim_signal, self.total_dim))
