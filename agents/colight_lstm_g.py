...
class CoLight_LSTM_G(IndependentAgent):
    def __init__(self, config, obs_act, map_name, thread_number):
        ...
        self.mhgat = [
                CoLightMultiHeadGATLSTM(self.head, self.latent_dim).to(self.device),  
                # nn.Linear(self.latent_dim, self.total_dim),
                # CoLightMultiHeadGATLSTM(self.head, self.latent_dim),
                # nn.Linear(self.latent_dim, self.total_dim),
                # CoLightMultiHeadGATLSTM(self.head, self.latent_dim),
                # nn.Linear(self.latent_dim, self.total_dim),
            ]
...
class CoLightAgent(nn.Module):
    ...
    def forward(self, x):
        # if len(x.shape) == 2:
        #     x = x.unsqueeze(0)
        target_index = x[:, -1, -1, 0]

        obs = x[:, :, :-1, :]                   # batch, n_history, signals, feat
        
        # batch_size = obs.shape[0]
        # dim_history = obs.shape[1]
        # dim_signal = obs.shape[2]
        # dim_feat = obs.shape[-1]
...
class CoLightMultiHeadGATLSTM(nn.Module):
    def __init__(self, n_heads, n_dim):
        super(CoLightMultiHeadGATLSTM, self).__init__()

        self.head = n_heads
        self.latent_dim = n_dim
        self.total_dim = n_heads * n_dim
        
        self.local_attn_heads = nn.LSTM(self.total_dim, self.total_dim, batch_first=True, num_layers=1)
        
        self.neighbor_attn_heads = nn.LSTM(self.total_dim, self.total_dim, batch_first=True, num_layers=1)
    ...
    def forward(self, embedded, adj_matrix):
        '''
        INPUT
        embedded    : (batch_size, history, n_intersection, feat)
        adj_matrix  : (n_intersection, direction, n_intersection)
        
        OUTPUT
        out         : (batch_size, n_intersection, feat)
        '''
        
        batch_size = embedded.shape[0]
        dim_history = embedded.shape[1]
        n_intersection = embedded.shape[2]
        dim_feat = embedded.shape[-1]

        n_direction = adj_matrix.shape[1]

        neighbor_embed = torch.tile(embedded.unsqueeze(2), (1, 1, n_intersection, 1, 1))    # (B, H, I, I, F)
        neighbor_embed = torch.matmul(adj_matrix, neighbor_embed)   # (B, H, I, D, F)

        # local head
        agent_heads = torch.permute(embedded, (0, 2, 1, 3))  # B, I, H, F
        agent_heads = torch.reshape(agent_heads, (batch_size * n_intersection, dim_history, dim_feat))   # B*I, H, F
        agent_heads, _ = self.local_attn_heads(agent_heads)    # B*I, H, F

        agent_heads = torch.reshape(agent_heads[:, -1, :], (batch_size, n_intersection, -1))  # B, I, F
        ...
        # Neighbor head
        neighbor_heads = torch.permute(neighbor_embed, (0, 2, 3, 1, 4))  # (B, H, I, D, F) -> (B, I, D, H, F) 
        neighbor_heads = torch.reshape(neighbor_heads, (batch_size * n_intersection, n_direction, dim_history, dim_feat))   # (B * I, D, H, F)
        neighbor_heads = torch.reshape(neighbor_heads, (batch_size * n_intersection * n_direction, dim_history, dim_feat))   # (B * I * D, H, F)
        neighbor_heads, _ = self.neighbor_attn_heads(neighbor_heads)    # B*I*D, H, F

        neighbor_heads = torch.reshape(neighbor_heads[:, -1, :], (batch_size * n_intersection, n_direction, -1))  # B*I, D, F
        neighbor_heads = torch.reshape(neighbor_heads, (batch_size, n_intersection, n_direction, -1))  # B, I, D, F
