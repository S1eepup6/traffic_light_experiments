...
class IndependentFrap(IndependentAgent):
    def __init__(self, config, obs_act, map_name, thread_number):
        ...
        for key in obs_act:
            if valid_acts is not None:
                pp_per_agent = [phase_pairs[i] for i in list(valid_acts[key].keys())]
            else:
                pp_per_agent = phase_pairs
            num_actions = len(pp_per_agent)

            comp_mask = []
            for i in range(len(pp_per_agent)):
                zeros = np.zeros(len(pp_per_agent) - 1, dtype=np.int)
                cnt = 0
                for j in range(len(pp_per_agent)):
                    if i == j: continue
                    pair_a = pp_per_agent[i]
                    pair_b = pp_per_agent[j]
            ...
            if valid_acts is not None:
                model = FRAP(config, num_actions, pp_per_agent, list(valid_acts[key].keys()), comp_mask, self.device)
            else:
                model = FRAP(config, num_actions, pp_per_agent, None, comp_mask, self.device)
            self.agents[key] = DQNAgent(config, num_actions, model)


class FRAP(nn.Module):
    def __init__(self, config, output_shape, phase_pairs, valid_acts, competition_mask, device):
