...
if __name__ == "__main__":
    ...
    egt = EGT_Layer(64, 64, 4)

    nodes = np.random.randn(1, 4, 64)
    nodes = torch.tensor(nodes, dtype=torch.float, device='cpu')

    edges = np.random.randn(1, 4, 4, 64)
    edges = torch.tensor(edges, dtype=torch.float, device='cpu')

    result = egt(nodes, edges, None)
