...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, net, state_fn, reward_fn, route=None, gui=False, end_time=3600,
        ...
        for ts in observations:
            if isinstance(observations[ts], list):
                self.obs_shape[ts] = [observations[ts][0].shape, observations[ts][1].shape]
            else:
    ...
    def calc_metrics(self, rewards):
        ...
        if self.tb_writer is not None:
            # self.tb_writer.add_scalar('reward/avg', sum(rewards.values()) / len(rewards), self.total_step)
            # self.tb_writer.add_scalar('max_queues/avg', sum(max_queues.values()) / len(max_queues), self.total_step)
            # self.tb_writer.add_scalar('queue_lengths/avg', sum(queue_lengths.values()) / len(queue_lengths), self.total_step)
            self.tb_writer.add_scalar('avg_values/reward', sum(rewards.values()) / len(rewards), self.total_step)
            self.tb_writer.add_scalar('avg_values/max_queues', sum(max_queues.values()) / len(max_queues), self.total_step)
            self.tb_writer.add_scalar('avg_values/queue_lengths', sum(queue_lengths.values()) / len(queue_lengths), self.total_step)
